...
def solve_Bellman_expectation(env, pi, gamma):
    ...
    for i, s in enumerate(states):
        # print(f"Processing state {s} ({i+1}/{n_S})")
        for a, pi_a in enumerate(pi[s]):
            # print(f"  Action {env.actions[a]} with probability {pi_a}")
            for s_next in env.successors(s):
                # print(f"    Next state {s_next} with probability {env.p(s_next, s, a)}")
                p = env.p(s_next, s, a)
                R_pi[i] += pi_a * p * env.reward(s_next, s, a)
...
def iterative_policy_evaluation(env, pi, gamma, max_iters=100, tol = 0, V_true=None):
    ...
    for i in range(max_iters):
        ...
        for s in env.states():
            V_new[s] = sum(pi[s][a] * sum(env.p(s_next, s, a) * (env.reward(s_next, s, a) + gamma * V_last[s_next])
                            for s_next in env.successors(s))
