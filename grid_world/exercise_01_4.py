...
from functions_01_3 import plot_V, solve_Bellman_expectation
...
for run in tqdm(range(n_runs), desc="Running MC experiments"):
    ...
    V_ev_monte_carlo, dist_ev_mc = ev_monte_carlo(env, pi, gam_0_9, s0=s0, n_samples=10000, T=128, V_true = V_pi)
    all_dist_ev_mc.append(dist_ev_mc)

    V_fv_monte_carlo, dist_fv_mc = fv_monte_carlo(env, pi, gam_0_9, s0=s0, n_samples=10000, T=128, V_true = V_pi)
    all_dist_fv_mc.append(dist_fv_mc)

# plot_V(env, V_ev_monte_carlo)
# plot_V(env, {state: V_pi[state] - V_ev_monte_carlo.get(state, 0) for state in V_pi.keys()})
